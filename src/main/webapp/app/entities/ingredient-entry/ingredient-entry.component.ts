...
@Component({
    selector: 'jhi-ingredient-entry',
    templateUrl: './ingredient-entry.component.html'
})
export class IngredientEntryComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/ingredient-entry'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/ingredient-entry', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: IngredientEntry) {
        return item.id;
    }
    registerChangeInIngredientEntries() {
        this.eventSubscriber = this.eventManager.subscribe('ingredientEntryListModification', (response) => this.loadAll());
    }
